...
st.caption("文部科学省公的データおよび主要塾の実費マスターに基づく学習費用試算")

# --- データ読み込み ---
@st.cache_data
def load_data():
    df_master = pd.read_excel("juku_evidence_master_v4.xlsx", sheet_name="収集データマスター", skiprows=3)
    df_mext = pd.read_excel("mext_juku_expenses_2023.xlsx", sheet_name="学年別学習塾費データ", skiprows=3)
...
def create_pptx_download(df_res, total_cost, route_label, cost_mode, selected_grades):
    prs = Presentation()
    ...
    # 3. カテゴリタグ (バッジ: OUTPUT)
    tag = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(0.4), Inches(2.2), Inches(0.35))
    tag.fill.solid()
    ...
    # 4. スライドタイトル
    txBox = slide.shapes.add_textbox(Inches(0.8), Inches(0.8), Inches(11.7), Inches(0.6))
    tf = txBox.text_frame
    p = tf.paragraphs[0]
    ...
    kpi_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.5), Inches(11.733), Inches(1.2))
    kpi_box.fill.solid()
    kpi_box.fill.fore_color.rgb = RGBColor(15, 23, 42)
    kpi_box.line.color.rgb = RGBColor(30, 41, 59)
    
    # KPIカード左アクセント線 (F59E0B)
    kpi_accent = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(1.5), Inches(0.12), Inches(1.2))
    kpi_accent.fill.solid()
    kpi_accent.fill.fore_color.rgb = RGBColor(245, 158, 11)
    kpi_accent.line.color.rgb = RGBColor(245, 158, 11)

    # KPIテキスト
    tx_kpi = slide.shapes.add_textbox(Inches(1.1), Inches(1.55), Inches(7.0), Inches(1.1))
    tf_kpi = tx_kpi.text_frame
    p_kpi_1 = tf_kpi.paragraphs[0]
    p_kpi_1.text = f"試算モデル：{route_label} （{len(selected_grades)}年間・{cost_mode}）"
    ...
    # 金額表示 (右寄せ)
    tx_val = slide.shapes.add_textbox(Inches(7.5), Inches(1.6), Inches(4.8), Inches(1.0))
    ...
    # カラム幅調整
    if cols == 5:
        col_widths = [Inches(1.8), Inches(2.0), Inches(2.0), Inches(2.2), Inches(3.733)]
    else:
    ...
    # ヘッダー設定
    headers = ["対象学年", "文科省平均(円)", "大手塾実費(円)", "算出費用 [幾何平均] (円)", "月額換算目安・時期"] if "中間ver" in cost_mode else ["対象学年", "算出費用(円)", "月額換算目安・時期", "算出根拠"]
    ...
    for r_idx, row in df_res.iterrows():
        bg_rgb = RGBColor(248, 250, 252) if r_idx % 2 == 1 else RGBColor(255, 255, 255)
        ...
        price_color = RGBColor(217, 119, 6) if row["is_peak"] else RGBColor(30, 58, 138)
    ...
    # 7. フッター注記
    tx_foot = slide.shapes.add_textbox(Inches(0.8), Inches(6.8), Inches(11.733), Inches(0.4))
...
def update_grade_selection():
    ...
    for g in ["小1", "小2", "小3", "小4", "小5", "小6", "中1", "中2", "中3", "高1", "高2", "高3"]:
        target_state = False
...
st.sidebar.checkbox("中学受験", key="target_chuju", on_change=update_grade_selection)
st.sidebar.checkbox("高校受験", key="target_kouju", on_change=update_grade_selection)
st.sidebar.checkbox("大学受験", key="target_daiju", on_change=update_grade_selection)

st.sidebar.divider()

cost_mode = st.sidebar.radio(
    "2. 費用感の算出基準",
    ["全平均ver（文科省データ）", "中間ver（幾何平均）", "大手ver（塾実費データ）"],
...
all_grades = ["小1", "小2", "小3", "小4", "小5", "小6", "中1", "中2", "中3", "高1", "高2", "高3"]

if "btn_小4" not in st.session_state:
    st.session_state["target_chuju"] = True
    st.session_state["target_daiju"] = False
    update_grade_selection()
...
if not selected_grades:
    st.warning("学年が1つも選択されていません。上部のチェックボックスから通塾学年を選択してください。")
else:
    results = []
    ...
    for grade in selected_grades:
        ...
        mext_val = mext_row[mext_spend_col].values[0] if not mext_row.empty else 0

        if grade in ["小1", "小2", "小3"]:
            route_filter = "中学受験（早期）"
        elif grade in ["小4", "小5", "小6"]:
            route_filter = "中学受験" if st.session_state.get("target_chuju", True) else "基礎固め・高校受験準備"
        elif grade in ["中1", "中2", "中3"]:
            route_filter = "高校受験"
        elif grade in ["高1", "高2", "高3"]:
            route_filter = "大学受験"
            
        master_filtered = df_master[(df_master["対象ルート"] == route_filter) & (df_master["対象学年"] == grade)]
        ...
        if grade in ["小6", "中3", "高3"]:
            peak_note = f"約 ¥{monthly_cost:,.0f}/月（受験本番：志望校特訓・直前対策等）"
            is_peak = True
    ...
    routes_selected = []
    if st.session_state.get("target_chuju"): routes_selected.append("中学受験")
    if st.session_state.get("target_kouju"): routes_selected.append("高校受験")
    if st.session_state.get("target_daiju"): routes_selected.append("大学受験")
    route_label = "・".join(routes_selected) if routes_selected else "カスタム選択"

    # 1. KPIカード
    st.markdown(f"""<div class="kpi-card"><div><div class="kpi-title">試算モデル：{route_label} （{len(selected_grades)}年間・{cost_mode}）</div><div class="kpi-sub">※選択された学年：{', '.join(selected_grades)}</div></div><div class="kpi-value">¥ {total_cost:,.0f} 円</div></div>""", unsafe_allow_html=True)

    # 2. テーブル（Markdown崩れ防止のため行頭インデント排除）
    ...
    if "中間ver" in cost_mode:
        ...
        for _, row in df_res.iterrows():
            price_class = "highlight-price-amber" if row["is_peak"] else "highlight-price"
        ...
        val_title = "文科省平均(円)" if "全平均" in cost_mode else "大手塾実費(円)"
    ...
    # 3. PPTX ダウンロードボタン
    if HAS_PPTX:
        pptx_file = create_pptx_download(df_res, total_cost, route_label, cost_mode, selected_grades)
        st.download_button(
        ...
        st.info("💡 `pip install python-pptx` を実行すると、パワポファイルのダウンロード機能が有効化されます。")